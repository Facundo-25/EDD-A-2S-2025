...
implementation
    ...
    uses
        gtk2, glib2, login, 
        userCreateCar, userViewVehicles,
    ...
    var
        ...
        btnAddVehicle, btnSelectService, btnReports, btnViewVehicles, btnLogout: PGtkWidget;
    ...
    procedure OnViewVehiclesClick(widget: PGtkWidget; data: gpointer); cdecl;
    begin
        userViewVehicles.ShowUserViewVehiclesWindow();
    end;
    ...
    begin
        ...
        gtk_window_set_default_size(GTK_WINDOW(userWindow), 300, 400);

        // Crear una tabla para organizar widgets (6 filas x 1 columna)
        ...
        btnViewVehicles   := gtk_button_new_with_label('Visualizar Vehículos');
        ...
        g_signal_connect(btnViewVehicles,  'clicked', G_CALLBACK(@OnViewVehiclesClick), nil);
        ...
        gtk_table_attach_defaults(GTK_TABLE(grid), btnViewVehicles, 0, 1, 2, 3);
        gtk_table_attach_defaults(GTK_TABLE(grid), btnSelectService,       0, 1, 3, 4);
        gtk_table_attach_defaults(GTK_TABLE(grid), btnReports,  0, 1, 4, 5);
        gtk_table_attach_defaults(GTK_TABLE(grid), btnLogout,        0, 1, 5, 6);
